...
Local usage: install zccache in a venv (`pip install zccache==1.14.8`), then
...
INSTALL_HINT = (
    "install it with `pip install zccache==1.14.8` (in a venv), "
    "`pipx install zccache==1.14.8` or `uv tool install zccache==1.14.8`"
)
...
def find_zccache() -> list[str] | None:
    ...
    so a caller can point it at e.g. `uvx --from zccache==1.14.8 zccache`
