...
Local usage: install zccache in a venv (`pip install zccache==1.14.6`), then
...
INSTALL_HINT = (
    "install it with `pip install zccache==1.14.6` (in a venv), "
    "`pipx install zccache==1.14.6` or `uv tool install zccache==1.14.6`"
)
...
def find_zccache() -> list[str] | None:
    ...
    so a caller can point it at e.g. `uvx --from zccache==1.14.6 zccache`
