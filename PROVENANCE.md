...
initially pruned too but had to be restored: `llvm/CMakeLists.txt` adds both unconditionally
inside its `LLVM_INCLUDE_UTILS` block, and configure failed without them, so both are present in
the payload today, byte-identical to upstream. One further file is restored on top of the prune:
