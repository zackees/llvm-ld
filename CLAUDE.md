...
cache, `zackees/zccache@1.14.6` for the job's cache group, `ci_jobs.py run` (the job under a zccache
...
```
python -m venv ~/.venvs/zccache && ~/.venvs/zccache/bin/pip install zccache==1.14.6   # or: pipx/uv tool install zccache==1.14.6
...
`ZCCACHE="uvx --from zccache==1.14.6 zccache"` works for replay/snapshot (not as the compiler
